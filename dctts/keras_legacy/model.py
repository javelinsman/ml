...
from tensorflow.keras.layers import Conv1D, Embedding, Input, Lambda
...
def mix_input(text_encoded_att, text_encoded_chr, audio_encoded, pass_audio_encoded):
...
class AudioDecoder:
    def __init__(self, pass_audio_encoded):
        self.pass_audio_encoded = pass_audio_encoded
        d = D['latent']
        self.layers = [
            Conv1D(d, 1, dilation_rate=1, padding="causal"),
            HighwayConv1D(3, 1),
...
class TTSModel:
    def __init__(self, pass_audio_encoded=False):
        self.text_encoder = TextEncoder()
        self.audio_encoder = AudioEncoder()
        self.audio_decoder = AudioDecoder(pass_audio_encoded)
        self.pass_audio_encoded = pass_audio_encoded
        self.model = self.__build()
    ...
    def __build(self):
        ...
        input_to_decoder, attention = \
            mix_input(text_encoded_att, text_encoded_chr, audio_encoded, self.pass_audio_encoded)
        ...
        def calc_attention_loss(attention):
        ...
        model.add_loss(Lambda(calc_attention_loss)(attention))
